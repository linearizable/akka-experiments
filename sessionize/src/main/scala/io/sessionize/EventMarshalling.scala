...
case class RequestDescription(request: String)
...
trait EventMarshalling  extends SprayJsonSupport with DefaultJsonProtocol {
  ...
  implicit val requestDescriptionFormat = jsonFormat1(RequestDescription)
  implicit val sessionFormat = jsonFormat7(Session.apply)
  implicit val sessionsFormat = jsonFormat1(Sessions)
